...
from FCC_Covid  import getCovidModel
...
TRAIN_IMG_DIR = parser.get('CL','train_img_dir_covid')
TRAIN_CSV_DIR = parser.get('CL','train_csv_covid')
VAL_IMG_DIR = parser.get('CL','val_img_dir_covid')
VAL_CSV_DIR = parser.get('CL','val_csv_covid')
...
def main():
    ...
    model, name = getCovidModel()
    model = model.to(DEVICE)
    loss_fn = nn.BCEWithLogitsLoss()
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)

    LOAD_MODEL_PATH = parser.get('CL','load_model_path_covid')
    SAVE_MODEL_PATH = parser.get('CL','load_model_path_covid')
    SAVE_IMAGES = parser.get('CL','save_images_covid')+"train/"
    SAVE_LOSS = parser.get('CL', 'save_loss_covid')
    ...
    train_loader, val_loader = get_loaders_covid_dataset(
        csv_train=TRAIN_CSV_DIR, img_dir_train=TRAIN_IMG_DIR, csv_val=VAL_CSV_DIR, img_dir_val=VAL_IMG_DIR,
        batch_size=BATCH_SIZE, train_transform=train_transform, val_transform=val_transforms, 
